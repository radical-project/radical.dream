...
class K8sCluster:
    ...
    def get_pod_logs(self, pod: Task, save: bool = False) -> str:
        ...
        # pod name that is generated by Hydraa only.
        #
        # FIXME: this command requires utils.misc.inject_kubeconfig
        #        to support attach kubeconfig to multiple kubectl binaries
        #        within the same cmd. Should we support it?
        #
        # cmd = 'kubectl get pods --no-headers=true -o custom-columns=":'
        # cmd += f'metadata.name" | grep "{pod.name}" | grep -v "worker" '
        # cmd += ' | xargs -I {} kubectl logs {}'
        cmd = f'kubectl logs {pod.name}'
        out, err, ret = sh_callout(cmd, shell=True, kube=self)
